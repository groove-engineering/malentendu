...
Not building: register-2/3 automation, a real graph database, or 600 atoms up front.
...
The schema *is* the method's attribution rule (§3), encoded in types. `source` is required on a
musicological claim; `held_by` is required on a felt/political position. The model is the spec;
the `.yaml` files are instances; the agent's structured output is validated against it.
...
One research agent, not yet an orchestrated "team": given a genre, it emits an `Atom` via
**structured output validated against the schema above** — so it *cannot* return a musicological
claim without a source. It fills the facts with **real, citable sources**, status `proposed`, and
opens a PR. One agent, one register, two genres. The "team" (an agent per aspect/register) comes
after the format holds.
...
Done = both atom sheets exist with sourced musicological claims, a maloya-x-footwork crossing
references them, and the circle has validated or contested at least the maloya sheet in a PR.
