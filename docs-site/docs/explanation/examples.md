...
The product is the **representation** (the source). Models are interchangeable. What we listen to goes back to fix the source — never the pipe.
...
We curate **atoms** (the ~600 genres), not molecules (the 360,000 fusions). Fixing an atom repairs all its crossings at once.
