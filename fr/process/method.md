...
> Spec **vivante**. C'est ici qu'on affine le modèle ; le PoC ([`../poc/`](../poc/))
> l'implémente une fois la spec stabilisée, pour éviter de bricoler le JSON à
> chaque idée. Voir aussi : [GENESIS.md](../GENESIS.md) (comment c'est né) ·
> [personas.md](personas.md) (qui décide) · [examples.md](examples.md) (diagrammes + exemples).

## 0. Principe fondateur

Le produit = **la méthode**, pas l'audio ni le prompt. Une fusion est décrite
**une seule fois**, indépendamment de tout modèle ; un compilateur la rend vers
une cible. **Suno / Udio / MusicGen / un musicien humain = backends interchangeables.**
...
Une fusion n'est pas que du son — c'est aussi des mots, et **le texte fusionne
aussi** (latin liturgique sur un kick séculier ; saudade portugaise sur du dub).
Deux couches à parts égales :
...
Chaque affirmation porte sa **source**. Le musicologique peut être vrai ou faux
(un expert tranche) ; le ressenti et le politique sont *tenus*, pas vrais. Un
atome de genre = un faisceau de **positions attribuées**, contestables. Deux
curateurs peuvent diverger : le moteur garde les deux. (Pas de vérité objective du
genre en dehors du registre musicologique.)
...
La boucle : on rend, l'oreille du cercle juge, et la correction revient à la **source** — un raté de genre corrige l'atome, un bel accident va au catalogue.
...
- **Texte** : plagiat, contenu explicite, imitation d'artiste réel, prononciation, authenticité vocale → déjà couvert par la chaîne **bitwize-music** (`lyric-writer`, `lyric-reviewer`, `plagiarism-checker`, `explicit-checker`, `pronunciation-specialist`, `voice-checker`).

## 6. La vision politique (registre 3)

> **Texte intégral :** [`political-vision.md`](political-vision.md) — les six thèses avec leurs références.

Salle des machines, pas paroles. Incarnée dans les croisements, la licence, la contrefaçon — presque jamais dite.

- **L'authenticité = un rapport de pouvoir** : on contrefait le réel pour exposer qu'il est *certifié*, pas essentiel. *(Debord, Benjamin.)*
- **Contre l'enclosure, pour les communs** : moteur libre, AGPL. *(Hyde.)*
- **Créolisation, pas lissage** : friction fertile contre le smoothie / le slop. *(Glissant.)*
- **Droit à l'opacité** : l'inclassable contre la lisibilité totale ; l'illisibilité comme résistance. *(Glissant, Scott.)*
- **Pas de dehors propre — auto-implication** : on utilise les armes de l'ennemi et on le dit. *(Debord ; voir GENESIS.)*
...
Enjeu nommé : **l'auto-implication.** Une IA fusionnant des musiques de cultures colonisées risque d'être l'extraction qu'elle critique. Réponse assumée : *« oui, et l'œuvre le sait — c'est le sujet. »*

## 7. Deux méta-garde-fous

- **La forme, pas le sermon.** La politique vit dans le geste, pas dans des paroles moralisatrices. Le slogan est récupérable ; le détournement ne l'est pas.
...
- **musicologique** → un musicologue / praticien expert. **Lacune actuelle : on en a besoin.**
...
Processus de décision complet : [personas.md](personas.md).
